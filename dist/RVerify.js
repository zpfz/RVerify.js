...
;(function (root, factory) {
  ...
  RVerify.action = function (callback) {
    if (Settings.extra !== null && typeof Settings.extra !== 'undefined') {
      var _extra;
      _extra = '<div class="rv-extra"><span>' + Settings.extraIcon + '</span><a href="' + Settings.extraLink + '" target="_blank" rel="noopener noreferrer">' + Settings.extra + '</a></div>';
    } else {
      _extra = '';
    }

    var modal = '<div class="rv-root"><div class="rv-mask"></div><div class="rv-wrap"><div class="rv-close">' + Settings.closeIcon + '</div><div class="rv-title">' + Settings.title + '</div><div class="rv-content"><div class="rv-text">' + Settings.text + '</div><div class="rv-image"><img src="' + getRandomImg(Settings.album) + '" draggable="false" class="rv-img"><div class="rv-image-mask"><svg t="1590812400512" class="rv-image-mask-success" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="2647" width="80" height="80"><path d="M927.97968 108.360629a50.575037 50.575037 0 0 0-69.085501 18.517689l-391.898737 678.933747-316.000056-182.409708A50.575037 50.575037 0 0 0 100.427574 711.005546l359.812488 207.690002a50.553362 50.553362 0 0 0 69.078276-18.517689L946.504593 177.44613a50.575037 50.575037 0 0 0-18.524913-69.085501z" fill="#ffffff" p-id="2648"></path></svg><svg t="1590815914523" class="rv-image-mask-error" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="3574" width="80" height="80"><path d="M441.23 511.44L79.4 149.62c-17.44-19.25-16.72-48.81 1.65-67.18 18.38-18.37 47.93-19.1 67.18-1.65l361.83 361.82L871.89 80.79a48.718 48.718 0 0 1 47.95-14.55 48.725 48.725 0 0 1 35.43 35.43 48.728 48.728 0 0 1-14.54 47.95L578.89 511.44l361.83 361.83a48.734 48.734 0 0 1 14.54 47.95 48.709 48.709 0 0 1-35.43 35.43 48.768 48.768 0 0 1-47.95-14.55L510.06 580.27 148.23 942.1c-19.25 17.44-48.8 16.72-67.18-1.65-18.37-18.37-19.09-47.93-1.65-67.18l361.83-361.83z" p-id="3575" fill="#ffffff"></path></svg></div></div><div class="rv-control"><div class="rv-bar">' + '</div><div class="rv-slider">' + Settings.sliderIcon + '</div></div>' + _extra + '</div></div></div>';
      ...
      document.onmouseup = function () {
        if (ReturnResult(getImgAngle())) {
          ...
          setTimeout(function () {
            ...
            slider.style.transition = 'background .2s ease-in-out,border-color .2s ease-in-out,box-shadow .2s ease-in-out,left .5s ease-in-out';
  ...
  function RandomAngle(tolerance) {
    if (tolerance < 5 || tolerance > 45) {
      ...
      return Math.floor(Math.random() * (360 - tolerance - 1 - (tolerance + 1))) + (tolerance + 1);
    }
  }

  // Get image angle
  function getImgAngle() {
    return parseFloat($('.rv-img').style['transform'].replace(/[^0-9\\.\\^0-9]/gi, ''));
  }
